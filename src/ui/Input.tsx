...
export interface InputProps
  extends Styleable,
    Changable<string | boolean | number>,
...
export function isInputElement(
  el: unknown
): el is React.ReactElement<InputProps> {
...
export const Field = React.forwardRef<HTMLLabelElement, FieldProps>(
  ({ className, label, error, children, ...props }, ref) => {
    const type = isInputElement(children) ? children.props.type : undefined;

    return FLAG_INPUTS.includes(String(type)) ? (
      <label {...props} ref={ref} className={className}>
        {children}
        <span>{label}</span>
      </label>
    ) : (
